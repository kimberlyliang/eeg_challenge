...
# my_submission.zip
# ├─ submission.py
# ├─ RandomForest_200_20251101_170036.joblib (or .pt)
...
class Submission:
    ...
    def get_model_challenge_1(self):
        ...
        # Try loading from .joblib files first (preferred for sklearn models)
        ...
        # Try loading from .pt files if .joblib didn't work
        if pipeline is None:
            for filename in model_filenames:
        ...
        if pipeline is None:
            ...
            raise FileNotFoundError(
                f"Could not load Random Forest model. Tried:\n{tried_files}\n"
                f"Please copy the Random Forest model file (e.g., from linear_models/) "
                f"to the submission directory."
            )
        
        # Wrap the pipeline in a PyTorch-compatible interface
