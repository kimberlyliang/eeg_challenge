...
# %%
# Program functions
def ingestion(Submission, data_dir, test_release="R6", fast_dev_run=False):
    ...
    print(f"Loading model on test dataset for challenge 1 (Release {test_release})")

    sub = Submission(SFREQ, DEVICE)
    model_1 = sub.get_model_challenge_1()
    model_1.eval()

    print(f"Loading test dataset for challenge 1 (Release {test_release})")
    dataset_1 = EEGChallengeDataset(
        release=test_release,  # Use different release for testing
        mini=False,
        query=dict(
    ...
    # Challenge 2
    # -----------
    print(f"Loading model on test dataset for challenge 2 (Release {test_release})")

    sub = Submission(SFREQ, DEVICE)
    model_2 = sub.get_model_challenge_2()
    model_2.eval()

    print(f"Loading test dataset for challenge 2 (Release {test_release})")
...
# %%
# Main
def main(submission_zip, data_dir, output_dir, test_release="R6", fast_dev_run=False):
    if fast_dev_run:
        print("WARNING: fast_dev_run is enabled, results will not be valid!\n")

    print(f"🧪 Testing on Release {test_release} (different from training release R5)")
    print("="*60)
    ...
    # Run ingestion
    ingestion_output = ingestion(Submission, data_dir, test_release, fast_dev_run)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="R6",
        help="Release to use for testing (should be different from training release).",
    )
    parser.add_argument(
        "--fast-dev-run",
        action="store_true",
    ...
    main(args.submission_zip, args.data_dir, args.output_dir, args.test_release, args.fast_dev_run)
