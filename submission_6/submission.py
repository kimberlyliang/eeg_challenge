...
from braindecode.models import EEGNeX, EEGConformer
import torch
...
class Submission:
    ...
    def get_model_challenge_1(self):
        model_challenge1 = EEGConformer(
            n_chans=129, n_outputs=1, sfreq=self.sfreq, n_times=int(2 * self.sfreq)
        ).to(self.device)

        model_challenge1.load_state_dict(torch.load(resolve_path("weights_challenge_1_conformer_11_01.pt"), map_location=self.device))
    ...
    def get_model_challenge_2(self):
        ...
        model_challenge2.load_state_dict(torch.load(resolve_path("weights_challenge_2_10_28.pt"), map_location=self.device))
